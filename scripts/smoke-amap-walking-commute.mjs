...
async function calculateWalkingCommute(origin, destination, anchorName) {
  const url = createWalkingUrl(origin.coordinate, destination.coordinate);
  const data = await fetchJson(url, `Walking commute to ${anchorName}`);

  if (data.status !== "1") {
    throw new Error(
      `Walking failed for ${anchorName}: ${data.info ?? "unknown error"} (${data.infocode ?? "no infocode"})`,
    );
  }
  ...
  if (!durationSeconds || !distanceMeters) {
    throw new Error(`Walking returned invalid duration or distance for ${anchorName}.`);
  }
  ...
  return {
    ...
    summary: `到「${anchorName}」步行约 ${durationMinutes} 分钟，路线距离约 ${(distanceMeters / 1000).toFixed(1)} 公里`,
  };
}
...
async function main() {
  ...
  console.log("Running Amap walking commute smoke test...");
  ...
  for (const sample of samples) {
    ...
    const commute = await calculateWalkingCommute(origin, destination, sample.anchorName);

    console.log("Walking commute:");
    console.log(JSON.stringify(commute, null, 2));
  }

  console.log("Amap walking commute smoke test finished.");
}
