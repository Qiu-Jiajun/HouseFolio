...
export const compareTableCopy = {
  ...
  fields: {
    ...
    commuteSource: "通勤来源",
    lifeCircle: "生活圈参考值",
    commuteSummaryCount: "通勤摘要数量",
    referenceScore: "参考评分",
    scoreBreakdown: "评分拆解状态",
    strengths: "优势信号",
    weaknesses: "劣势信号",
    neutralFacts: "中性事实",
    subjectiveSummary: "主观评分摘要",
    hasNotes: "是否有笔记",
    hasPhotos: "是否有照片",
    photoCount: "照片数量",
    missingFields: "待补充字段",
  ...
  values: {
    yes: "有",
    no: "无",
    hasScoreBreakdown: "已有拆解",
    noScoreBreakdown: "暂未拆解",
    noSignals: "暂无",
    noMissingFields: "字段较完整",
    noRiskFlags: "暂无明显信号",
  },
} as const;
...
export const compareExplanationCopy = {
  ...
  sections: {
    ...
    commute: {
      title: "通勤会长期影响日常稳定性",
      body:
        "通勤时间会持续影响上班、上学和日常安排。如果某套房源在通勤上更有优势，仍需要结合采光、噪音、合同条款和实际看房体验一起判断。",
    },
    confidence: {
      title: "资料完整度会影响判断置信度",
      bodyWithSignals:
        "当前对比中存在待补充字段或需要人工确认的信号。它们只表示需要进一步核实，不代表房源真假判断，也不构成租赁建议。",
      bodyWithoutSignals:
        "当前没有明显的待补充字段或风险信号，但这只代表结构化信息相对完整，不代表房源已经被验证。最终仍需要你自行核实。",
    },
    checklist: {
      title: "下一步建议核实",
      items: [
